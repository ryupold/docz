...
public class SelectDatabaseDialog extends javax.swing.JDialog {
    ...
    private void btnDeleteDBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteDBActionPerformed
        if (JOptionPane.showConfirmDialog(this, "Are you sure? The complete folder '" + databases.elementAt(listDatabases.getSelectedIndex()) + "' will be deleted!", "Delete database", JOptionPane.YES_NO_OPTION)
    ...
    private void txtNewDBPathCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_txtNewDBPathCaretUpdate
        if (!txtNewDBPath.getText().trim().isEmpty()) {
            btnAddDB.setEnabled((!new File(txtNewDBPath.getText().trim()).exists() || new File(txtNewDBPath.getText().trim()).isDirectory()) && !databases.contains(txtNewDBPath.getText().trim()));
        }
