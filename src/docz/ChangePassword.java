...
public class ChangePassword extends javax.swing.JDialog {
    ...
    private void checkFields(){
        ...
        if(pwdOldPW.getPassword().length == 0){
            lblStatus.setText("Enter the old password");
        }
        else if(pwdOldPW.getPassword().length > 0 && pwdNewPW.getPassword().length == 0){
            lblStatus.setText("Enter the new password");
        }
        else if(pwdOldPW.getPassword().length > 0 && pwdNewPW.getPassword().length > 0 && pwdConfirm.getPassword().length == 0){
    ...
    private void btnOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOKActionPerformed
        try {
            if(DB.changePW(new String(pwdOldPW.getPassword()), new String(pwdConfirm.getPassword()))){    
                setVisible(false);
            }
            else{
                lblStatus.setForeground(Color.red);
